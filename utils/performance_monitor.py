...
class PerformanceBenchmark:
    ...
    def benchmark_query(self, name: str, query: str, params: tuple = None, iterations: int = 10) -> Dict[str, Any]:
        ...
        print(f"  ✅ Avg: {avg_time:.3f}ms, Min: {min_time:.3f}ms, Max: {max_time:.3f}ms, Results: {results_count}")
    ...
    def generate_report(self):
        ...
        sorted_results = sorted(self.results, key=lambda x: x.get('avg_time_ms', 0), reverse=True)
        
        print(f"  Query Performance Summary:")
        print(f"  {'Query Name':<25} {'Avg (ms)':<10} {'Min (ms)':<10} {'Max (ms)':<10} {'Results':<8}")
        ...
        for result in sorted_results:
            ...
            print(f"  {name:<25} {avg_time:<10.3f} {min_time:<10.3f} {max_time:<10.3f} {count:<8}")
        
        # Performance categories
        total_queries = len(self.results)
        fast_queries = len([r for r in self.results if r.get('avg_time_ms', 0) < 10])
        medium_queries = len([r for r in self.results if 10 <= r.get('avg_time_ms', 0) < 100])
        slow_queries = len([r for r in self.results if r.get('avg_time_ms', 0) >= 100])
        
        print(f"\n  Performance Distribution:")
        print(f"  • Fast queries (<10ms):    {fast_queries}/{total_queries} ({fast_queries/total_queries*100:.1f}%)")
        print(f"  • Medium queries (10-100ms): {medium_queries}/{total_queries} ({medium_queries/total_queries*100:.1f}%)")
        print(f"  • Slow queries (>100ms):   {slow_queries}/{total_queries} ({slow_queries/total_queries*100:.1f}%)")
