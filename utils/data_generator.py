...
class DataGenerator:
    ...
    def __init__(self):
        ...
        self.states = [
            'NY', 'CA', 'IL', 'TX', 'AZ', 'PA', 'FL', 'OH', 'IN', 'NC', 'WA', 'CO',
            'MA', 'TN', 'MI', 'OK', 'OR', 'NV', 'KY', 'MD', 'WI', 'NM', 'LA', 'MN'
        ]
        
        self.product_adjectives = [
            'Premium', 'Deluxe', 'Professional', 'Advanced', 'Smart', 'Ultra', 'Super',
            'Mega', 'Pro', 'Elite', 'Ultimate', 'Enhanced', 'Digital', 'Wireless',
            'Portable', 'Compact', 'Heavy-Duty', 'Industrial', 'Commercial', 'Eco-Friendly'
        ]
        
        self.product_nouns = [
            'Widget', 'Device', 'Tool', 'Gadget', 'Accessory', 'Kit', 'Set', 'Pack',
            'System', 'Solution', 'Equipment', 'Apparatus', 'Instrument', 'Machine',
            'Component', 'Module', 'Unit', 'Assembly', 'Hardware', 'Software'
        ]
    ...
    def generate_customers(self, count: int = 1000) -> int:
        ...
        for i in range(count):
            ...
            customers_data.append((first_name, last_name, email, phone, address, city, state, zip_code))
    ...
    def generate_products(self, count: int = 500) -> int:
        ...
        categories = self.db.execute_query("SELECT category_id, category_name FROM categories")
        if not categories:
        ...
        for i in range(count):
            ...
            sku = f"{category['category_name'][:4].upper()}-{random.randint(1000, 9999)}"
            
            products_data.append((product_name, description, price, stock_quantity, sku, category['category_id']))
    ...
    def generate_orders(self, count: int = 2000) -> int:
        ...
        products = self.db.execute_query("SELECT product_id, price FROM products WHERE stock_quantity > 0")
        ...
        for i in range(count):
            ...
            try:
                ...
                order_id = self.db.execute_update_with_id(
                    order_query, 
                    (customer['customer_id'], random_date, status, 0.00)
                )
                ...
                for _ in range(num_items):
                    ...
                    self.db.execute_update(
                        item_query,
                        (order_id, product['product_id'], quantity, unit_price, total_price)
                    )
