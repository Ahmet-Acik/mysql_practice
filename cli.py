...
class MySQLPracticeCLI:
    ...
    def setup_database(self):
        ...
        try:
            ...
            subprocess.run([
                sys.executable, 
                str(self.project_root / "create_database.py")
            ], check=True)
    ...
    def generate_data(self, count: int = 1000):
        ...
        try:
            subprocess.run([
                sys.executable, str(data_gen_file), 
                "--count", str(count)
            ], check=True)
    ...
    def interactive_mode(self):
        ...
        while True:
            ...
            if choice == "0" or choice.lower() == "quit":
                ...
                example_type = input("Enter example type (basic/advanced/transactions/procedures/all): ").strip()
                if not example_type:
                    example_type = "all"
                self.run_examples(example_type)
            elif choice == "3" or choice.lower() == "exercises":
                level = input("Enter exercise level (beginner/intermediate/advanced/all): ").strip()
                if not level:
                    level = "all"
                ...
                count = input("Enter number of records to generate (default: 1000): ").strip()
                try:
...
Examples:
  ...
  python cli.py benchmark                # Run performance tests
    ...
    parser.add_argument(
        ...
        nargs="?", 
        choices=[
            "setup", "examples", "exercises", "tests", 
            "api", "analytics", "generate", "benchmark", "status"
        ],
