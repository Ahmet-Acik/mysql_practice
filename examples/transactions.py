    ...
    def transaction_best_practices_demo(self):
        ...
        try:
            ...
            if current_stock < order_quantity:
                print(f"   ✗ Insufficient stock (need {order_quantity}, have {current_stock})")
                return
            ...
            order_rows = self.db.execute_update(create_order_query, (customer_id, total_amount))
            ...
            if order_rows > 0:
                ...
                """
                stock_rows = self.db.execute_update(update_stock_query, (order_quantity, product_sku))
                
                if stock_rows > 0:
                    print(f"   ✓ Stock updated (new quantity: {current_stock - order_quantity})")
                else:
            ...
            try:
                ...
                if locks_result:
                    ...
                    for lock in locks_result:
                        print(f"   - {lock['OBJECT_NAME']}: {lock['LOCK_TYPE']} ({lock['LOCK_MODE']})")
                else:
